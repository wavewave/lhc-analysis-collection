...
p_sqsg_5j2x :: DCross 
p_sqsg_5j2x = x (t proton, t proton, [p_gluino, p_sdownR])


idx_sqsg_5j2x :: CrossID ProcSmplIdx
idx_sqsg_5j2x = mkCrossIDIdx (mkDICross p_sqsg_5j2x)

map_sqsg_5j2x :: ProcSpecMap
map_sqsg_5j2x = 
  ...
  getConfig fp >>= 
    maybe (return ()) (\ec -> do 
      ...
      evchainGen ADMXUDD112
        ssetup 
        ("Work20130527_sqsg","sqsg_5j2x") 
        param 
        map_sqsg_5j2x p_sqsg_5j2x 
        mgrs 

      let wsetup' = getWorkSetupCombined ADMXUDD112 ssetup param ("Work20130527_sqsg","sqsg_5j2x")  mgrs 
          wsetup = wsetup' { ws_storage = WebDAVRemoteDir "montecarlo/admproject/XUDD/scan_sqsg_5j2x" } 
