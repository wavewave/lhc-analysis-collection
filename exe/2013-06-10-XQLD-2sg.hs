...
worksets = [ (mgl,msq,50000,50000, 10000) | mgl <- [700], msq <- [100,200,300,400] ] 

-- mgl <- [100,200..2000], msq <- [100,200..2000] ] 

--   [ (mgl,msq,50000,50000, 10000) | mgl <- [1000], msq <- [1200] ] 
  ...
  getConfig fp >>= 
    maybe (return ()) (\ec -> do 
      ...
      evchainGen ADMXQLD111
        ssetup 
        ("Work20130421_2sg","2sg_2l4j2x") 
        param 
        map_2sg_2l4j2x p_2sg_2l4j2x 
        mgrs 

      let wsetup' = getWorkSetupCombined ADMXQLD111 ssetup param ("Work20130421_2sg","2sg_2l4j2x")  mgrs 
          wsetup = wsetup' { ws_storage = WebDAVRemoteDir "montecarlo/admproject/XQLD/scan_2sg_2l4j2x" } 
