...
main = do 
  ...
  -- forM_ datalst (getCount.createRdirBName "2sq_4j2x") --  ( "sqsg_5j2x") -- ("2sg_6j2x")

  -- checkthefiles "2sq_4j2x" -- "sqsg_5j2x"  -- "2sg_6j2x"
  

  h <- openFile "xudd_sqsg_data.dat" WriteMode 
   
  mapM_ (\(mg,msq,r)-> hPutStrLn h (mg ++ ", " ++ msq ++ ", " ++ r))
    =<< forM datalst
              (\(x,y) -> do 
                 t_2sq <- (getLHCresult.createRdirBName "2sq_4j2x") (x,y) 
                 t_sqsg <- (getLHCresult.createRdirBName "sqsg_5j2x") (x,y)
                 t_2sg <- (getLHCresult.createRdirBName "2sg_6j2x") (x,y)
...
createRdirBName procname (mg,mq) = 
  let rdir = "montecarlo/admproject/XUDD/scan_" ++ procname 
      basename = "ADMXUDD112MG"++mg++ "MQ" ++ mq ++ "ML50000.0MN50000.0_" ++ procname ++ "_LHC7ATLAS_NoMatch_NoCut_AntiKT0.4_NoTau_Set"
  in (rdir,basename)  
